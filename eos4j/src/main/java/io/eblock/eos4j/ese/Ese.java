...
public class Ese {
	 ...
	 * @return
	 */
	public static String parseEOSWINONE(String star,String con,String tag) {
		DataParam[] datas = new DataParam[] {
				new DataParam(star, DataType.unit64, Action.account),
				new DataParam(con, DataType.string, Action.account),
				new DataParam(tag, DataType.string, Action.account),
		};
		byte[] allbyte = new byte[] {};
		for (DataParam value : datas) {
			allbyte = ByteUtils.concat(allbyte, value.seria());
